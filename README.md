...
アップロード対象フォルダには `src/`、`public/`、`functions/`、`worker/`、`scripts/`、設定ファイル、README、AGENTS、TASKSを含めます。`node_modules/`、`dist/`、`out/`、`.npm-cache/`、`.npm-pack-cache/`、`.manual-rollup/`、`.wrangler/` は含めません。
...
- 標準ルール分析と無料AI要約による主要トピック、注意語句、数値候補、根拠ページの提示
- AI貼り付け用プロンプト、Markdownレポート、根拠ページリストのコピー
- Yahoo!ファイナンスの銘柄コード検索へのリンク
- localStorageによる履歴保存
...
画面内の「無料AI要約」は、PDFから抽出した文章を端末内で抽出・整理する機能です。Gemini、OpenAI、Claudeなどの外部LLM APIへ自動送信しません。

画面の「AI貼り付け用プロンプトをコピー」は、ユーザーが外部AIのWeb画面へ手動で貼り付けるための文章を作る機能です。アプリ内部から外部AIに送信しません。
...
このフォルダには `functions/api/proxy.ts` を同梱しています。Cloudflare Pagesへこのリポジトリを接続すると、同一オリジンの `/api/proxy?url=...` が利用でき、TDnet公開ページやTDnet PDFのCORS制限を回避しやすくなります。
...
Cloudflare Pages Functionsを使わず、別Workerを使いたい場合は `worker/` のCloudflare Workers proxyをデプロイし、アプリの設定画面へWorker URLを入力してください。
