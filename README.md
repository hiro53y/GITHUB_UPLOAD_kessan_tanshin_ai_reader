...
決算短信AIリーダーは、日本株の銘柄コードを入力して、TDnet公開閲覧ページとJPX「東証上場会社情報サービス」から最新の決算関連資料をベストエフォートで探し、PDFテキスト抽出と標準ルール分析で読みどころを整理するAndroid Chrome向けPWAです。
...
TDnet公開閲覧ページは無料公開ページを対象にしたベストエフォート取得です。2026年6月20日時点では、検索画面または一覧HTMLから以下の構造を利用します。
...
TDnetの検索画面で決算関連資料が見つからない場合は、JPX「東証上場会社情報サービス」の会社別開示履歴へ自動的にフォールバックします。Cloudflare Pagesでは `functions/api/proxy.ts` と `functions/api/disclosures.ts` を同一オリジンAPIとして自動利用するため、通常は外部Worker URLの入力は不要です。

設定した検索期間が120日未満で決算資料が見つからない場合、最新決算の探索だけは120日まで自動拡張します。たとえば5451（ヨドコウ）は30日設定でも、2026年5月11日公表の2026年3月期決算短信を取得対象にします。

TDnet/JPX側のHTML構造変更、CORS制限、掲載期間、ネットワーク状況により自動取得は失敗する可能性があります。失敗時は正常ケースとして扱い、手動PDFアップロードまたはPDF URL貼り付けで続行できます。
...
Cloudflare Pagesで公開したアプリは、同梱のPages Functionsを使ってTDnet・JPX・PDF配信元へアクセスします。別のオリジンで公開する場合、Workers AI要約を使う場合、またはPages Functionsが利用できない構成では、`worker/` のCloudflare Workers proxyを別途デプロイし、アプリの設定画面へWorker URLを入力してください。
...
ブラウザからTDnet・JPX・PDFへ直接アクセスできない場合は、`worker/src/index.ts` をCloudflare Workersへ配置し、設定画面にWorker URLを入力します。Workerは `/disclosures` でJPX会社別開示履歴も取得します。

Worker proxyの方針:

- 許可ドメインのみproxy
- 初期許可は `www.release.tdnet.info`、`release.tdnet.info`、`www2.jpx.co.jp`
- 任意URL proxyはしない
- 簡易レート制限あり
